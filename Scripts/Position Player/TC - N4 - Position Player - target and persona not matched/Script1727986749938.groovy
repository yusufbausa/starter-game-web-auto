...
TestData excelDataPersona = findTestData('Data Files/Create Persona/Create Persona')
String valueFromExcelPersona = excelDataPersona.getValue(2,1)

GlobalVariable.persona = valueFromExcelPersona
log.logInfo("Global Variable value : " + GlobalVariable.persona)

TestObject persona = findTestObject('Object Repository/Attack Player/option_persona')
personaTag = WebUI.modifyObjectProperty(persona, 'xpath', 'equals', "//div[@role='option'][contains(.," + '"' + GlobalVariable.persona + '"' + ")]" , true)
//div[@role='option'][contains(.,'${persona}')]
...
boolean verifyPosition = WebUI.verifyElementPresent(positionResponse, 5)


if (verifyPosition == true) {
	log.logInfo("Updating position with target and persona didn't matched")
	KeywordUtil.markFailed("Test Failed : Position shouldn'e be updated")
} else {
	log.logInfo("Updating position with target and persona didn't matched")
